...
function UnpublishDialog(props) {
  ...
  const unpublishPartner = async () => {
    setLoading(true);
    try {
      const partnersService = new props.partners();
      const partnersData = await partnersService.unpublishPartner();
  ...
  return showError == true ? (
    <Modules.ErrorDialog
      {...props}
      title="Erreur Survenue Lors De La Dépublication"
    ...
    >
      ...
      <Modules.DialogActions>
        ...
        <Modules.SnackbarComponent
          {...props}
          title="Titre"
          message="Succès De La Dépublication !"
          open={openSnackbar}
        />
      </Modules.DialogActions>
    </Modules.Dialog>
